  ...
  */
  public void inputInsert(){
	  ...
	  /* if(result == InsertResult.INSERT_OUTINDEX)
	   ...
	   else if(result == InsertResult.INSERT_OUTINDEX)
		  EndView.printMessage(code+"는 중복이므로 등록할수 없습니다.");
	   else 
           EndView.printMessage("상품이 등록되었습니다.");*/
       
       switch(result) {
        case INSERT_OUTINDEX : EndView.printMessage("더이상 등록할 수 없습니다."); break;
        case INSERT_DUPLICATE : EndView.printMessage(code+"는 중복이므로 등록할수 없습니다."); break;
        default : EndView.printMessage("더이상 등록할 수 없습니다.");
       }
  }
